...
### Time for an example... ###
I’m going to walk you through implementing a solution to a typical scenario that benefits from the use of Service Broker, the complete project for this example can be downloaded from github at <a href="https://github.com/gavdraper/ServiceBrokerTicketMaster">https://github.com/gavdraper/ServiceBrokerTicketMaster</a>. I recommend you clone the example and have a look at the source code as I’m not going to cover everything here. 
...
<a href="/content/images/WPImport/2012/06/image.png"><img style="background-image: none; border-right-width: 0px; padding-left: 0px; padding-right: 0px; display: inline; border-top-width: 0px; border-bottom-width: 0px; border-left-width: 0px; padding-top: 0px" title="image" border="0" alt="image" src="/content/images/WPImport/2012/06/image_thumb.png" width="518" height="551"></a>

1. A user hits the “Create Booking” button on the web application to create a booking.  
1. The webserver inserts the booking record into the SQL Server database.  
1. The Webserver sends a message to the payment processor target queue to process the payment for that booking.  
1. The payment processor service will pick the message up and process the payment  
   1. If the payment is a success it sends a payment success message back to the payment processor initiator queue.  
   1. If it fails it sends a fail message back
1. SQL Server is monitoring the payment initiator queue for payment responses.  
	1. If a payment fails it sets the booking record to payment failed.  
	1. If the payment was a success it sets the booking table to payment success and sends a message to the ticket printer target queue.
1. The ticket printing service picks up the message in the ticket printer target queue and prints the ticket.  
	1. If this succeeds it sends a success message back to the ticket printer initiator queue  
	1. If it fails it sends a failed message.
1. SQL Server picks up the message from the ticket printer initiator queue and with sets the ticket to printed or failed.
...
If you look at the full <a href="https://github.com/gavdraper/ServiceBrokerTicketMaster">example project</a> on github you can see I then have a console app that picks up this message, there are a few ways you can achieve this.
...
1. Pulls a message off the ProcessPaymentInitiator queue.
1. Parses the message and updates the booking to say if the payment succeeded.
1. Sends the end conversation message back to end the payment conversation.
1. If the payment was a success it will send a message to the PrintTarget queue instructing the booking to have it’s ticket printed.
...
The best way to see all this in action is to clone and build the source from <a href="https://github.com/gavdraper/ServiceBrokerTicketMaster">github</a>, then run both the print and payment console apps, once they are running run the webapp and make a booking. You will then be able to see the messages flow from the webapp to the payment console and then to the print console.